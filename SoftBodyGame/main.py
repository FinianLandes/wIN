...
if __name__ == "__main__":
    ...
    body = SoftBody(points=points, n_sub=10, edge_alpha=0.0005, area_alpha=0.01, diag_alpha=0.005, damp=0.05)
    collider = LineCollider(point=np.array([0.0, 0.0]), normal=np.array([0.0, 1.0]), restitution=0.95)
    ...
    while running:
        ...
        body.step(dt)
        collider.collide(body.points)
        screen.fill((20, 20, 40))
        body_render.draw(screen)
        pg.display.flip()
